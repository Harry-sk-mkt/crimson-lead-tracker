 ...
 * v1.7.0 (2026-08-09)
 * - **F/H/J/L(SAL%/IC Booked%/IC Complete%/Won%)와 CPNP1/New P1 Target%를
 *   값 대신 실제 시트 수식으로 전환**(30_ACQReport.js v1.16.0과 동일 이유,
 *   사용자 요청). F/H/J/L은 `buildRatioFormula_(...,"")`(분모 0 → 공란,
 *   기존과 동일), CPNP1은 분자(Spent) 공란 가드, New P1 Target%는 분모
 *   (Target) 공란 가드로 `buildGuardedRatioFormula_()`(54_Events_Write.js
 *   정의 재사용, guardColLetter 인자로 어느 쪽을 가드할지 지정).
 * v1.6.2 (2026-08-06)
...
function generateNewP1Report_(){
  ...
  reversedTargetRows.forEach(function(row, i){
    ...
    // 시트 행 번호(수식의 셀 참조에 필요).
    const sheetRow = CONFIG.NEWP1.ROWS.REPORT_DATA_START + i;
    const spentColLetter = columnIndexToLetter_(CONFIG.NEWP1.TARGET_COLUMNS_START_COL);
    const newP1TargetColLetter = columnIndexToLetter_(CONFIG.NEWP1.TARGET_COLUMNS_START_COL + 2);
    ...
    outputRows.push([
      ...
      newP1,
      row.sal,
      buildRatioFormula_("E", "D", sheetRow, ""),
      row.icBooked,
      buildRatioFormula_("G", "D", sheetRow, ""),
      row.icComplete,
      buildRatioFormula_("I", "D", sheetRow, ""),
      row.won,
      buildRatioFormula_("K", "D", sheetRow, ""),
      row.revenue
    ]);
    ...
    // CPNP1/New P1 Target%는 값 대신 수식(buildGuardedRatioFormula_)으로 이
    // 구분을 그대로 재현(2026-08-09, 사용자 요청 — 수동 입력값 수정 시 자동
    // 재계산). CPNP1은 분자(Spent, N열)가 외부 조회값이라 그 셀이 공란인지로
    // 가드(guardColLetter="N"), New P1 Target%는 ACQ_REP과 동일하게 분모
    // (Target, P열) 공란 여부로 가드.
    const hasSpent = spendMap.hasOwnProperty(key);
    const spent = hasSpent ? spendMap[key] : "";
    const cpnp1 = hasSpent
      ? buildGuardedRatioFormula_(spentColLetter, "D", sheetRow, spentColLetter) : "";
    ...
    const newP1TargetPct = hasNewP1Target
      ? buildGuardedRatioFormula_("D", newP1TargetColLetter, sheetRow) : "";
