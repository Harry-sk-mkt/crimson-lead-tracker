 ...
 * v1.10.0 (2026-08-09)
 * - RATIO_FORMULAS 신규 — GROUP_5_DERIVED 6개 컬럼의 분자/분모 짝을
 *   중앙화(기존엔 53_Events_Merge.js의 applyGroup5Derived_() 함수 본문에
 *   하드코딩돼 있었음). 이 값이 JS에서 나눗셈을 계산하는 대신 실제 시트
 *   수식을 쓰는 데 쓰임(사용자 요청 — 수동 입력값 수정 시 자동 재계산).
 *   Config Centralization 원칙(Article 11) 준수.
 * v1.9.0 (2026-08-06)
...
const EVENTS = {
  ...
  */
  ...
  /*
  ==========================================================
  RATIO FORMULAS (2026-08-09 신규)

  GROUP_5_DERIVED 각 컬럼의 분자/분모 짝 — 54_Events_Write.js의
  buildRatioFormula_()가 이 스펙으로 시트 수식을 생성한다(값 계산
  대신). 순서/짝은 기존 applyGroup5Derived_()(53_Events_Merge.js,
  이번에 삭제됨) 본문과 동일 — 출력값 변경 없음.
  ==========================================================
  */
  RATIO_FORMULAS: [

    { column: "Match Rate", numerator: "SF Reg.", denominator: "Mkt Reg." },
    { column: "Success %", numerator: "Success", denominator: "Mkt Reg." },
