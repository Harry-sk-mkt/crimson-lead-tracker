...
/**
 * ==========================================================
 * Compute BOFU Funnel Aggregates (IC Request/Booked/Complete/Deals/Revenue)
 *
 * WHY
 * "모든 리포트는 Leads_OPS를 읽는다" 원칙 유지 — Leads_OPS에서 그대로
 * 읽고 leadIdToKey(Leads_Master 기준, First Touch)로 조인해 First Touch
 * Attribution을 보장한다 (Events와 동일 하이브리드 패턴).
 *
 * TEST
 ...
 * ==========================================================
 */
function aggregateBOFUFunnelRecords_(opsRecords, leadIdToKey, icRequest, icBooked, icComplete, dealsWon, revenue) {
...
function testComputeBOFUFunnelAggregates_() {
  ...
  aggregateBOFUFunnelRecords_(opsRecords, leadIdToKey, icRequest, icBooked, icComplete, dealsWon, revenue);

  const pass =
    icRequest["WF-2025-07-KOR-BOFU-Core A"] === 1 &&
    icBooked["WF-2025-07-KOR-BOFU-Core A"] === 1 &&
    revenue["WF-2025-07-KOR-BOFU-Core A"] === 500 &&
