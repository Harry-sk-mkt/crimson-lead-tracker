...
Append 함수에 자동 연결함 (`MASTER_001_IncrementalMasterBuild.js`):
...
**동작 (`applyICRequestTracking_()`, `OPS_004_Merge.js`)**: `mergeOPS()`가 실행될 때마다(= 매 OPS
...
`OPS.ROWS = { HEADER: 1, DATA_START: 2 }` — `OPS_001_Config.js`에 추가됨.
`OPS_005_Write.js`의 `writeOPS()`와 `OPS_002_Styles.js`의 `applyOPSStyle()` 모두 하드코딩된 `1`, `2` 대신
이 값을 참조한다.

## Technical Modules
(2026-08-09 네이밍 컨벤션 전환 — 구 `2x_OPS_*.js` → 신규 `OPS_00x_*.js`, 파일 내용 변경 없음)
- `OPS_001_Config.js` — Sheet names, Header definitions, Column groups, Constants, Rows
- `OPS_002_Styles.js` — 서식 적용
- `OPS_003_Build.js` — Main build entry point
- `OPS_004_Merge.js` — Email lookup, Merge logic, Preserve manual columns
- `OPS_005_Write.js` — Write merged results, Formatting, Metadata update
- `OPS_006_QA.js` — `Leads_OPS_QA` Dashboard/Issues 생성 (위 "QA Output" 참고)
