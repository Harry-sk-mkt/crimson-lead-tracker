# FY_REP Design — 채택 안 된 원래 설계 (보존 기록)

> 이 문서는 `docs/FYReportDesign.md`에서 2026-09-15에 분리됐습니다. 2026-07-30 당시
> 검토했던 원래 설계 초안 전체이며, **채택되지 않고 폐기됨** — 실제 FY_REP은 2026-08-07에
> 완전히 다른 구조(Marketing/ACQ/Pipeline/Revenue 4섹션, 세그먼트 7개)로 재착수돼
> `FYREP_001_Engine.js`/`FYREP_002_Report.js`로 구현 완료됐습니다(`docs/exec-plans/
> completed/2026-08-07-fy-rep-implementation.md` 참고). 현재 설계를 확인하려면 이 파일이
> 아니라 그 exec-plan과 실제 코드 파일을 볼 것 — 아래 내용은 역사적 기록일 뿐입니다.
...
## 9. 다음 단계 (당시 기록 — 실제로는 아래 대신 완전히 새 구조로 재착수됨)
