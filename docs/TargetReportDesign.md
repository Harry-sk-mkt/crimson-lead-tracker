...
## 4. Week / Calendar Rules (2026-07-27 확정, 월 귀속 규칙 2026-08-07 수정)

- **주 = 월요일~일요일** (세일즈마케팅 회의 리포팅 기간 기준, 변경 불가 제약)
- **주의 월 귀속 = 그 주 7일 중 과반(4일 이상)을 차지하는 달** (`getWeekMonthLabel_()`,
  90_TargetEngine.js) — **2026-08-07 수정**: 원래는 "그 주의 월요일이 속한 달"이었으나, 월
  경계에 걸친 주(예: 2026-08-31(월)~09-06(일) — 월요일만 8월, 나머지 6일은 9월)가 월요일
  하루만 보고 8월로 잘못 분류되던 문제 발견(사용자 리포트) — Target/Actual CPNP1이 월 단위
  값을 그 달 모든 주에 반복 표시하는 구조라, 실제로는 9월인 주에 8월 값이 표시되는 결과로
  이어짐. 한 주(7일)는 최대 한 번만 월 경계를 넘을 수 있어 과반 기준은 항상 그 주 목요일
  (월요일+3일)이 속한 달과 일치. **FY 귀속도 같은 과반 기준으로 통일**(`getWeekFiscalYear_()`,
  90_TargetEngine.js) — 처음엔 월만 고치고 FY는 월요일 기준으로 남겨뒀으나, 검토 중 "FY와
  월 귀속 기준이 다르면 아주 드물게(Aug 1이 화~목요일에 걸리는 해) 한 FY 리포트 안에 AUG가
  처음/끝 두 번 나타나고 Ad_Spend_Cache 조회 키(FY|Month)도 어긋날 수 있다"는 구조적 위험이
  발견돼 FY 귀속도 과반 기준으로 확장(사용자 확인, 2026-08-07) — node 스크립트로 FY25~32
  전체 시뮬레이션해 매 FY 첫 주 AUG/마지막 주 JUL 유지 + 인접 FY 사이 공백·중복 0 확인.
- **그 달의 주 수 = 위 과반 규칙으로 그 달에 귀속된 주의 개수** (항상 4 또는 5)
...
- **리포트 영역**: 행 = 대상 FY의 월~일 주 전체(52~53행). 미래 주는 Target만, Actual 공란
  (**2026-08-07 버그 수정**: Actual CPNP1은 월 단위 값을 그 달 모든 주에 반복 표시하는
  구조라, 이 "미래 주는 공란" 규칙이 실제로는 지켜지지 않고 아직 시작하지 않은 주에도
  월 누적치가 그대로 표시되고 있었음(사용자 리포트) — `generateTargetReport_()`/
  `updateTargetReportActuals_()`(91_TargetReport.js)에 "이번 주 월요일보다 이후인 주는
  Actual CPNP1을 공란 처리" 가드 추가로 해소. Actual P1은 Leads_OPS를 직접 카운트하는
  구조라 애초에 미래 주엔 항상 0으로 정상 표시되고 있었음, 변경 없음)
  → 연간 페이스 표가 미리 완성된 형태
...
| 지표 | New P1, CPNP1 (2개) |
| 주 정의 | 월~일 고정 (세일즈마케팅 회의 기준), 월 귀속=과반(4일 이상, 2026-08-07 수정 — 이전엔 월요일), 주 수=그 규칙으로 귀속된 주 개수 |
