...
53. **Events_OPS "Time"(I열, GROUP_1_MANUAL) 유실 — 복구 완료, 근본 결함(release 소유권 미확인) 발견·수정 완료, 실사용 검증 대기(TODO)** (2026-09-18 등록, 사용자 보고) — 사용자가 Events_OPS I열에 입력해둔 Time 값이 사라졌다고 보고. `TEMPQA_062_EventsTimeColumnDiagnostic.js`(`runCheckEventsTimeColumnDiagnostic()`, 신규)로 전수 스캔한 결과 361행 중 11행만 Time 값 존재, 350행 공란 — 그 중 247행은 PIC/Speaker/Mkt Reg.(GROUP_2_MANUAL)가 정상 값으로 채워진, 사용자가 분명히 관리해온 행들이라 "애초에 Time을 입력한 적 없는 행"이 아니라 **유실**로 확인. 남아있는 11건의 Time 값 자체도 서로 무관한 이벤트 7개가 초 단위까지 완전히 동일한 시각(`1899-12-30T02:02:08.000Z`)을 공유하는 등 정상적인 개별 수동입력으로 보기 어려운 흔적(드래그 채우기/붙여넣기 잔재로 추정, 확정 아님).

    **원인(조사 중 — 최초 가설 정정됨)**: 사용자가 "9/17 11:54 AM엔 Time 전체가 있었다", "동일 날 3:58 PM엔 이미 사라져 있었다"고 확인. Apps Script Executions 로그 대조 결과 `periodicRefreshRevenue_`(Time-Driven)가 **3:55:34 PM 시작, 179.653초 소요 → 약 3:58:33 PM 종료** — 사용자가 유실을 확인한 시각과 정확히 일치. 코드 확인: `periodicRefreshRevenue_()` → `runRevenuePipelineTail()` → `refreshOPSSheets_(type, state)` → **`buildEventsOPS()`**(`MASTER_002_PipelineAsync.js:2761-2763`) 호출 경로 존재. **최초 가설(사용자가 그 순간 Time을 라이브로 입력 중이었을 것)은 사용자 확인으로 폐기** — 문제의 Time 값들은 그날 입력된 게 아니라 최소 2주 이상 안정적으로 존재해온 데이터였음. 즉 "스냅샷 시점에 아직 저장 안 된 라이브 입력값을 놓침"이 아니라, **`buildEventsOPS()`가 스냅샷을 뜨는 그 순간 시트의 I열이 이미(다른 원인으로) 비어있었을 가능성**이 더 유력 — 실제 유실 시점은 3:58 PM보다 더 이를 수 있고(예: 같은 날 11:51:55 AM~12:17:39 PM에 실행된 `runLeadsPipelineTail`의 `refreshOPSSheets_` 단계), 그 뒤 `periodicRefreshRevenue_`(1:51 PM/3:55 PM) 등 후속 `buildEventsOPS()` 실행들은 이미 비어있는 상태를 그대로 반복해서 다시 썼을 뿐일 수 있음. `buildEventsOPS()`(`EVENTS_003_Build.js`)의 "스냅샷 읽기 → 병합 → `sheet.clear()` 후 전체 재작성" 구조 자체는 `#52`(Leads_OPS SAL/IC Date 유실)와 동일한 취약점이지만, **정확히 무엇이 스냅샷 이전에 I열을 비웠는지는 아직 미확정** — 후보: (1) 같은 종류의 스크립트 실행끼리 겹침(`#52`와 동일 클래스, `guardPipelineTailEntry_()`가 커버 못 하는 사각지대가 남아있을 가능성), (2) 시트에서의 수동 편집(사람의 실수), (3) 아직 특정 못 한 제3의 스크립트. 다음 조사 시 Google Sheets 버전 기록(파일 > 버전 기록)에서 11:54 AM~12:17 PM 구간의 정확한 편집 주체(사람 vs Apps Script)를 대조할 것 — `#52` 조사 때와 동일한 방법론.

    **구조적 함의(임의로 처리하지 말 것)**: 같은 "read existing → merge → `sheet.clear()` 후 전체 재작성" 패턴을 Leads_OPS(`OPS_004_Merge.js`)뿐 아니라 Events_OPS/BOFU_OPS/Search_OPS/Content_OPS(각 `*_004_Merge.js`)가 전부 동일하게 씀(`EVENTS_004_Merge.js` 헤더 주석에 "22_OPS_Merge.js의 mergeOPS() 패턴을 그대로 따름"이라고 명시) — 즉 이 4개 OPS 시트 전부가, 어떤 이유로든(사람의 실수든 스크립트끼리의 경합이든) 스냅샷 시점에 값이 비어있으면 그 뒤 통째로 재작성하며 그 빈 상태를 고착시킬 수 있는 구조적 위험을 안고 있음. **미수정** — 가능한 방향(전부 미검증 아이디어일 뿐, 임의로 착수하지 말고 설계 논의 먼저): (1) 편집 중 rebuild를 지연시키는 락/디바운스, (2) 전체 clear+rewrite 대신 변경된 셀만 갱신하는 diff 기반 쓰기로 전환, (3) Manual 컬럼 그룹은 아예 건드리지 않고 Computed/Derived 컬럼만 부분 재작성.

    **복구 완료(2026-09-18, 사용자 확인)**: Google Sheets 파일 > 버전 기록에서 Time 값이 남아있던 시점의 스냅샷으로 Events_OPS를 업데이트해 I열 복구 완료. 버전 기록의 편집 주체는 "사용자 본인"으로 표시됐으나, 이 프로젝트는 설치형 트리거(Full Authorization)로 동작해 스크립트의 자동 실행도 트리거를 설치한 계정 명의로 기록되므로 이것만으로 사람/스크립트를 구분할 수 없음 — `periodicRefreshRevenue_`/`buildEventsOPS()` 실행 시각과의 상관관계(위 참고)가 스크립트 쪽에 더 무게를 둠. **정확히 어떤 실행이 스냅샷 시점에 I열을 비웠는지는 여전히 미확정** — 재발 시 위에 남긴 조사 방법(Executions 전체 목록 대조, 버전 기록 편집 시각 대조)으로 계속 좁힐 것.

    **근본 결함 발견 및 수정 완료(2026-09-18, 실사용 검증 대기)**: 사용자와 함께 코드를 다시 짚어보다, `releasePipelineLockAndProcessQueue_()`(`MASTER_002_PipelineAsync.js`)가 호출하는 `releasePipelineLock_()`이 **"지금 저장된 락이 정말 내가 잡은 그 락인지" 확인 없이 무조건 삭제**하고 있었음을 발견 — 사람이 아무것도 수동 실행하지 않아도 순수 자동 트리거 타이밍만으로 재현 가능한 결함(#52와 별개의 세 번째 락 버그 후보). 시나리오: 오래 걸리는 tail(예: `runLeadsPipelineTail` 실측 1543초, `LOCK_STALE_THRESHOLD_MS` 30분에 근접)의 락은 front door(import 시점)에 획득된 것이라 실제 나이가 tail 실행시간보다 길 수 있음 → 다른 트리거(`periodicRefreshRevenue_` 등)가 이를 죽은 락으로 보고 self-heal하며 자기 타입으로 덮어씀 → 원래 tail이 끝나며 호출하는 release가 "지금 락이 이미 다른 타입 것"인데도 확인 없이 지워버려, 아직 실행 중인 그 다른 tail의 보호막이 사라지고 대기열의 세 번째 타입까지 끼어들 수 있음 — 이 상태에서 두 개 이상의 `buildEventsOPS()`(또는 `buildLeadsOPS()`/`buildBOFUOPS()`/`buildSearchOPS()`/`buildContentOPS()`)가 동시에 돌면 `#52`/이번 Time 유실과 동일한 클래스의 스냅샷~재작성 경합 발생.

    **수정(`MASTER_002_PipelineAsync.js` v1.35.0, clasp push 완료)**: `releasePipelineLock_(expectedLockValue)`를 compare-and-delete로 변경 — 호출자가 진입 시점에 스냅샷해둔 락 원본 문자열(`currentPipelineLockValue_()`)과 현재 저장된 값이 다르면 삭제를 건너뛰고 `false` 반환(로그만 남김), 같으면 삭제 후 `true`. `expectedLockValue`를 생략하면(수동 강제 해제 전용 진입점) 기존과 동일하게 무조건 삭제 — 하위호환 유지. `releasePipelineLockAndProcessQueue_(expectedLockValue)`도 동일 값을 받아 전달하고, 반납이 스킵되면 대기열 처리도 함께 건너뜀(내 것이 아닌 락 위에서 대기열을 재배정하면 또 다른 경합을 만들 수 있어서). `runLeadsPipelineTail`/`runMTAPipelineTail`/`runICFunnelPipelineTail`/`runSALPipelineTail`/`runRevenuePipelineTail` 5개 tail 함수 전부가 guard/acquire 통과 직후 자기 락 값을 스냅샷해 release 호출에 넘기도록 수정. 신규 테스트 `testReleasePipelineLockOwnershipCheck()`(PropertiesService 실제 읽기/쓰기로 3가지 케이스 검증 — 정상 반납/소유권 불일치 시 스킵/강제 해제 하위호환) — **사용자 실행 결과 PASS 확인(2026-09-18, case1/2/3 전부 true)**.

    **실사용 재검증 필요** — 이 수정 이후에도 같은 유실이 재발하지 않는지, 그리고 락 소유권 불일치로 반납이 스킵되는 로그가 실제로 남는 상황이 있는지 다음 Import들을 통해 계속 지켜볼 것. 완료로 간주하지 말 것.

54. **UTM_Program_Dictionary 등록폼 접미사 미정규화 버그 — 근본 원인 수정 완료, override 후보 23건 검토는 보류(TODO)** (2026-09-18 등록, 사용자 보고: Events_OPS 신규 WB 웨비나 7건 Spent 미반영) — `TEMPQA_063_EventsMetaSpendMissingDiagnostic.js` 조사 결과, `aggregateUtmProgramCounts_()`(`UTIL_002_UtmProgramDictionary.js`)가 "ㅣRegistered for Webinar from FB LG Form"/"...from Website Form" 등록폼 접미사를 안 뗀 원본 Lead Source Detail/First Touch Detail 값으로 `distinctProgramCount`(모호 여부 판정)를 계산하고 있었음 — 실제로는 완전히 같은 프로그램인데 접미사 변형만 있어도 서로 다른 프로그램으로 오판돼 `readUtmProgramDictionaryMap_()`이 통째로 제외(딕셔너리를 소비하는 Events/BOFU/Content Meta 자동매칭 전체에 영향). 이 프로젝트의 다른 모든 소비처(`resolveMetaCampaignEventsKey_()` 등, `EVENTS_002_Engine.js`)는 이미 조회 후 `stripLGSuffix_(stripRegistrationFormSuffix_(...))`로 접미사를 떼는데, 정작 "모호함 판정" 시점에만 이 정규화가 빠져있던 불일치.
    **수정(`UTIL_002_UtmProgramDictionary.js` v1.13.0, clasp push 완료)**: `aggregateUtmProgramCounts_()`가 카운트 키를 만들 때도 동일 정규화 적용. 신규 테스트 `testAggregateUtmProgramCountsNormalizesRegistrationFormSuffix()`(접미사만 다른 변형은 합쳐지고, 진짜 다른 프로그램끼리는 여전히 구분되는 것 둘 다 확인) 통과. 기존 저장 캐시(hidden JSON 컬럼)는 정규화 전 원본 키를 담고 있어 증분 갱신만으론 해소 안 돼 `runRefreshUtmProgramDictionary()` 전체 재구축 1회 필요 — 사용자가 실행 완료.
    **✅ 실사용 검증 완료(2026-09-18, 사용자 확인)**: 재구축 후 Events 7건 중 6건 자동 해소($2,849~$5,091 NZD 정상 매칭). 나머지 1건("WB-2026-06-KOR-MOFU-Core EA/ED Application Strategy")은 접미사 문제가 아니라 진짜 다른 프로그램(California Dream/Rise Stanford Roadmap)과 소수(11~15%) 섞인 케이스라 `META_CAMPAIGN_NAME_TO_EVENTS_KEY_OVERRIDE`(`EVENTS_002_Engine.js` v1.22.0)에 수동 override 2건 추가로 해결 — 7/7 전부 매칭 확인.
    **✅ Events/BOFU/Content 전체 재검토 완료(2026-09-18, 사용자 요청)**: `TEMPQA_064_PostFixMetaCoverageRecheck.js`(`TEMPQA_051`의 범용 커버리지 진단을 Events까지 확장)로 확인 — "확실한 후보인데 매칭 안 됨"(진짜 버그) 버킷이 Events 0건/Content 0건(9/8에 별도 원인으로 이미 해소된 상태 유지, 회귀 없음)/BOFU 1건("Hyperlocalized Korean Army Infographic" — `docs/OpenItems_Legacy.md` #30에 이미 기록된 진단 스크립트 자체의 알려진 한계, 실제로는 Content로 정상 재분류돼 있어 조치 불필요)로 전부 정상 확인.
    **미해결 — "모호하지만 실제 Meta 지출 존재"(override 후보) 23건 검토는 보류(사용자 확정)**: Events 4건/BOFU 12건/Content 7건이 EA/ED와 동일한 패턴(진짜 다른 프로그램과 소수 섞여 다수결로도 못 뚫림)으로 새로 확인됨 — `docs/OpenItems_Legacy.md` #30에 이미 "override 맵 도입 여부는 사용자 결정 필요, 임의로 처리하지 말 것"으로 기록됐던 것과 동일 항목. 오늘은 기록만, 검토·override 추가는 사용자가 나중에 요청 시 진행(`runTraceEventsAmbiguousMetaSpendCandidates()`류 패턴으로 도메인별 경쟁 Program 후보를 먼저 확인 후 결정).
