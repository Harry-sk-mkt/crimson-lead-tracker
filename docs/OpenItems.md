...
19. **캠페인 지출(Ad_Spend_Cache) 독립 스케줄 갱신 — 보류(TODO, 2026-08-04 기록)** — `refreshAdSpendCache_()`
    는 2026-08-04부터 `08_PipelineAsync.js`의 배경 파이프라인 체인(`refreshCampaignSpend_()`,
    Leads/MTA 신규 데이터가 들어와 파이프라인이 돌 때 같이 실행)에 연결됨. 리드 유입과 무관하게
