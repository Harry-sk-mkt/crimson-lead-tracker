...
**다음 세션 시작점(2026-09-09 세션 종료 시점)**:
1. **🟡 진행 중 — 실행이 오래 걸려 사용자가 도중에 취소함(2026-09-09), 다음 세션
   최우선 확인 사항**: `MASTER_012_RawExternalMigration.js` v1.2.0에
   `runDeleteLegacyMainRawBackups()`/`computeLegacyRawBackupDeletionPlan_()` 신규
   구현(안정화 6일 확인 후 사용자가 삭제 진행 확정) — 외부 스프레드시트 행 수가 메인
   백업 이상일 때만 삭제하는 안전장치 포함, 확인 dialog(YES/NO) 통과 후 삭제하는
   구조. 사용자가 Apps Script 편집기에서 실행했으나 오래 걸려 완료 전에 취소함 —
   **다음 세션 시작 시 재실행 전에 반드시 Apps Script 편집기 Executions 탭에서 이
   실행이 실제로 끝났는지/어디까지 진행됐는지부터 확인할 것**(`docs/apps-script-
   gotchas.md` #5 — 서버 쪽은 취소돼도 이미 끝나 있거나 계속 진행 중일 수 있음,
   성급한 재실행은 중복 실행 위험). 재실행 자체는 안전 — 이미 삭제 대상 판정
   로직이 메인 시트가 없으면(`mainRowCount === 0`) "이미 없음"으로 자동 스킵하므로
   일부만 삭제된 상태에서 다시 실행해도 나머지만 마저 처리됨. 느려진 원인은
   미확인(외부 파일 3개 `openById()`+`getLastRow()` 자체는 가벼운 메타데이터 호출이라
   오래 걸릴 이유가 약함 — YES/NO 확인 dialog가 뜬 채로 클릭을 기다리는 상태를
   "멈춘 것"으로 오인했을 가능성이 유력, 다음 세션에 실행 로그 소요시간 직접 확인
   필요).
2. ~~`RESET_001_ResetRawMaster.js` 헤더 주석 — "Raw 시트 직접 비우기" 설명을 외부
   스프레드시트 기준으로 갱신~~ — ✅ 완료(2026-09-09).
3. ~~`TEMPQA_040`/`TEMPQA_042`/`TEMPQA_043`(IC Funnel 조사 스크립트) — 이관 후 참조가
   깨질 것, 삭제해도 되는지 사용자 확인 필요~~ — ✅ 완료(2026-09-09, 사용자 확인 후
   3개 전부 삭제).
4. ~~`docs/OpenItems.md` #45(Salesforce 필드 인벤토리 문서화)/#46(installable onEdit
   재발동 이슈)~~ — 둘 다 이 exec-plan과 무관하게 이미 별도 세션에서 완료된 것으로
   확인됨(2026-09-09 재확인) — 이 항목 자체가 stale, 조치 불필요.
5. "전체 행 읽기/쓰기 배치 처리" 설계(Progress 체크리스트 참고) — 청크 크기/구현 방식
   미정으로 남겨둠, 실측상 급하지 않음(87,180행도 단일 호출로 103초). 사용자가
   2026-09-09에 "삭제 결과부터 확인 후 결정"하기로 함 — 1번 항목 마무리 후 착수
   여부 다시 논의.
