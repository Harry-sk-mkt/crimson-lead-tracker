...
## Surprises & Discoveries (계속, 실 시트 검증 중 발견)

- **Block 0/Block A 컬럼 충돌 버그**: Block 0에 신규 월별 그리드 섹션(B~M열, 12개월)을 추가하면서
  `BLOCK_A_START_COL`(원래 D열=4)이 그 그리드 한복판과 정확히 겹치는 걸 놓쳤음 — 사용자가 실제
  시트에서 "Monthly Company-wide Inputs 행에 AUG/SEP 다음 다른 블록(Webinar MAR...)이 이어짐"을
  발견해 확인됨. 실제로는 Block 0와 Block A가 같은 행의 서로 다른 컬럼을 각자 쓰고 있었을 뿐이지만,
  Block A가 그 컬럼에서 자기 값으로 덮어써서 Block 0가 쓴 값이 사라지는 실질적 데이터 손실 버그였음.
  **수정 완료(2026-07-30)**: Block A~D 시작 컬럼을 전부 +10 이동(4/13/21/28 → 14/23/31/38, 기존
  블록 간 1칸 간격 패턴 유지). Node 하네스 재실행으로 회귀 없음 확인(단, 이 버그 자체는 컬럼 번호
  충돌이라 하네스로는 못 잡음 — 실 시트 검증에서만 발견 가능했던 유형).
- **컬럼 이동의 부작용**: 이전 실행에서 이미 옛 컬럼 위치(D~AB열 등)에 Block A~D 데이터가 쓰여
  있었으므로, 컬럼을 옮긴 뒤 `refreshTargetEngine_()`의 wide-clear(새 `BLOCK_A_START_COL`인 14부터
  시작)가 옛 위치(4~13열)의 잔재를 안 지움 — 그 열은 이제 Block 0 소유 영역이라 wide-clear가
  의도적으로 건드리지 않기 때문. **권장 조치**: 다음 재실행 전 Target_Engine 시트 전체를 완전히
