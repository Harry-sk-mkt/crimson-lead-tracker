...
## 13. `clasp push`는 "파일 삭제만" 있는 변경을 조용히 무시한다 (2026-09-16 실측, clasp 3.3.0)

로컬에서 파일을 삭제하고 `clasp push`(`-f`/`--force` 포함)를 실행해도, **그 push에서 로컬↔원격
차이가 "파일 삭제"뿐이면 clasp는 실제 API 호출(`updateContent`)을 아예 건너뛰고 "Script is
already up to date."만 출력한다** — 삭제가 조용히 실패하는데 성공한 것처럼 보인다.

**원인**: clasp의 `push` 명령은 먼저 `getChangedFiles()`로 "push할 게 있는지" 확인하는데, 이
함수는 **로컬 파일 목록만 순회**하며 각각을 원격과 비교한다(`files.js:304`, "새 파일이거나 내용이
다르면 변경"). 원격에만 존재하고 로컬엔 없는 파일(=삭제 대상)은 애초에 이 순회에 들어가지 않아서
"변경"으로 잡히지 않고, 결과가 빈 배열이면 `push()`(실제 `updateContent` 호출부, 여기선 로컬
파일 전체를 보내 원격을 통째로 교체하므로 정상적으로는 삭제도 반영됨)가 아예 호출되지 않는다.
즉 **같은 push에 다른 실제 파일 변경이 하나라도 섞여 있으면 삭제도 같이 정상 반영되지만, "삭제만"
단독으로 push하면 100% 무시된다.**

**실측 사례**: `docs/OpenItems.md` #52 조사 중 `TEMPQA_060_ICFunnelBookedCompletedFullBackfill.js`
(사용 후 삭제됐다고 Changelog에 기록됨)가 로컬엔 없는데 원격 컨테이너엔 그대로 남아있는 걸 발견 —
`clasp push`/`clasp push -f` 둘 다 "already up to date"만 출력, 재pull로 원격에 여전히 파일이
있음을 확인.

**대응**: `scripts/safe-clasp-push.sh`가 매 push 후 임시 디렉토리에 원격을 pull해 파일명을 로컬과
diff하고, 로컬에 없는 원격 전용 파일이 있으면 경고하도록 수정(2026-09-16). 경고가 뜨면 이 스크립트로
재시도해도 같은 이유로 계속 무시되므로, **Apps Script 편집기에서 해당 파일을 직접 삭제**하는 것이
유일하게 확실한 방법.