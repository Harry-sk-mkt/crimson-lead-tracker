...
- **Manual Execution Instructions**: `clasp run-function` 미도입 상태라 Apps Script 함수는 사용자가 Apps Script 편집기에서 직접 Run 해야 한다. 사용자에게 함수 실행을 요청할 때는 **반드시 파일명 + 함수명을 함께 명시**한다 (예: "`MASTER_003_MTAFunnelSync.js`의 `runSyncMTAFunnelToOPS()` 실행해주세요" — 함수명만 말하지 않는다).
...
- **Session-End One-Sentence Task Summary**: 위 Changelog 기록/커밋과 함께, 채팅 응답에도 그 세션에서 한 일을 태스크 단위로 한 문장씩 정리해서 보여준다 — Changelog는 상세 기록용, 채팅 요약은 사용자가 그 자리에서 빠르게 훑어볼 용도로 별도 필요(2026-07-28 사용자 요청). **형식(2026-07-31 확정, 2026-09-23 보강)**: 프로즈 문단이 아니라 `-`로 시작하는 불릿 리스트, 한 문장 = 한 줄. 문장은 **개조식 명사형 종결**(`~완료`, `~수정`, `~삽입` — `~했습니다` 서술형 금지, 부연 설명 없이 결과만) — 루트 `crimson/CLAUDE.md` "Session-End Summary Format"과 동일. **범위(2026-08-20 확정)**: 불릿은 실제 작업(코드 변경/버그 수정/조사 결과)만 나열 — "문서 정리하고 커밋했다"류의 Changelog 기록·커밋·push 자체는 매 세션 당연히 하는 절차이지 보여줄 성과가 아니므로 별도 불릿으로 넣지 않는다.
- **Session-Start Git Sync Check (+ 자동 Pull)**: **트리거는 "세션 시작" 그 자체이지 "코드 수정 임박"이 아니다** — 새 세션(또는 오랜만의 재개)에서 사용자의 첫 메시지가 "어디까지 했었지?" 같은 순수 상태 확인/질문이라도, 코드를 한 줄도 안 건드릴 예정이라도 상관없이 그 세션의 **첫 응답을 만들기 전에** `scripts/start-session.sh`부터 실행한다(git fetch/divergence, `git worktree list`, pre-commit hook 설치 여부를 한 번에 확인 — 배경: 2026-07-24 divergence 미인지 사고, 2026-07-29 worktree 덮어쓰기 사고. 상세 이력은 스크립트 주석 참고). "코드를 수정하기 전에"라는 표현은 "그 시점까지는 안 돌려도 됨"이라는 유예 조건이 아니라 "이 체크 없이는 코드 수정 자체를 시작하지 않는다"는 최종 마감 기한을 뜻할 뿐이다(2026-09-02 사고 — 첫 메시지가 상태 확인 질문이라 "아직 코드 안 건드리니 나중에"로 잘못 판단, 로컬이 origin보다 3커밋 뒤처진 걸 모른 채 오래된 정보로 답변한 뒤에야 사용자 질문으로 뒤늦게 발견됨. 재발 방지로 트리거 조건 명문화). **로컬이 origin보다 뒤처지기만 하고(ahead 없음) 로컬에 커밋 안 된 변경사항도 없으면, 스크립트가 확인 없이 바로 `git pull`까지 자동 진행한다**(fast-forward만 가능한 안전한 상황이므로) — 로컬에 uncommitted 변경이 있거나 ahead/behind가 동시에 있는(진짜 divergence) 경우엔 자동 pull하지 않고 사용자에게 먼저 알린다(2026-08-07 도입, 매 세션 수동으로 "pull 됐는지" 물어봐야 했던 번거로움 방지 목적).
- **Clasp Push Pre-Authorized / 세션 간 권한 유지**: `clasp push`는 이 프로젝트의 일상적 배포 단계이므로 코드 수정 완료 후 매번 진행 여부를 묻지 않고 바로 실행한다 — git push(원격 저장소, 협업자와 공유됨 — 여전히 명시 요청 시에만)와는 별개로 취급한다. 이 승인은 세션 간에도 유지되는 것으로 간주 — harness 권한 프롬프트가 뜨는 작업은 `.claude/settings.json` 허용 목록에 등록해 재확인 자체를 없앤다 (2026-07-28 사용자 확정). **단, `clasp push`를 직접 실행하지 말고 반드시 `scripts/safe-clasp-push.sh`를 통해 실행한다** — worktree가 2개 이상이면 목록을 보여주고 y/n 확인을 받은 뒤에만 push한다 (2026-07-29 worktree 덮어쓰기 사고 재발 방지, 상세는 스크립트 주석 참고). **"묻지 않고 바로 실행"이지 "생략해도 됨"이 아니다** — 코드 수정을 마쳤다고 판단되는 시점, 특히 사용자에게 Apps Script 함수 실행을 요청하기 직전에는 그 전에 반드시 `scripts/safe-clasp-push.sh`부터 먼저 실행했는지 스스로 확인한다. 깜빡하고 push 없이 실행을 요청하면 컨테이너엔 예전 코드가 그대로 남아있어, 사용자가 함수를 실행해도 변경사항이 하나도 반영 안 된 것처럼 보여 혼란을 준다 (2026-08-06 실제 발생 — Events_OPS 헤더 개편 코드를 수정하고 push를 깜빡한 채 `buildEventsOPS()` 실행을 요청, 사용자가 실행 결과 옛 헤더 그대로인 것을 보고서야 push 누락을 발견함).
