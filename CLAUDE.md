...
3. MTA_Master에 "완전 동일한(all-fields identical) duplicate row" 검출 로직 없음 — 현재는 MTA의 row 중복(1 Lead = N Touch)을 전부 의도된 것으로 간주해 dedup을 아예 하지 않음. Salesforce export가 실수로 완전 동일한 touch row를 중복 전달하는 경우를 걸러내지 못함. 판단 기준(어떤 필드 조합을 "완전 동일"로 볼지)부터 정의 필요 — 2026-07-22, TODO로 보류. (`findDuplicateTouchRows_()`로 Lead ID+MTA Created Date 조합 기준 3,401개 그룹 발견했으나, Raw 자체가 날짜 단위라 정상 다중 터치와 진짜 중복을 구분할 필드가 없음.)
