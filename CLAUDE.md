...
- **Session-End One-Sentence Task Summary**: 위 Changelog 기록/커밋과 함께, 채팅 응답에도 그 세션에서 한 일을 태스크 단위로 한 문장씩 정리해서 보여준다 — Changelog는 상세 기록용, 채팅 요약은 사용자가 그 자리에서 빠르게 훑어볼 용도로 별도 필요(2026-07-28 사용자 요청). **형식(2026-07-31 확정)**: 프로즈 문단이 아니라 `-`로 시작하는 불릿 리스트, 한 문장 = 한 줄. **범위(2026-08-20 확정)**: 불릿은 실제 작업(코드 변경/버그 수정/조사 결과)만 나열 — "문서 정리하고 커밋했다"류의 Changelog 기록·커밋·push 자체는 매 세션 당연히 하는 절차이지 보여줄 성과가 아니므로 별도 불릿으로 넣지 않는다.
