 ...
 * v1.36.0 (2026-08-09)
 * - `CONFIG.ACQ.CPNP1_COLUMN`(24, X열) 신규 — 사용자가 이미 ACQ_REP에
 *   수동으로 만들어둔 CPNP1(=Spent÷New P1) 컬럼 위치. 코드는 이 컬럼의
 *   값/수식을 전혀 쓰지 않고(수동 영역 원칙 유지) On Track 하이라이트만
 *   적용(30_ACQReport.js/32_ACQReportStyles.js 참고, 사용자 요청).
 * v1.35.0 (2026-08-08)
...
const CONFIG = {
  ...
  ACQ: {
    ...
    // CPNP1(=Spent÷New P1) — 사용자가 W열 바로 다음(X열)에 이미 수동으로
    // 만들어둔 컬럼(헤더 "CPNP1", X4). 코드는 값/수식을 전혀 쓰지 않고
    // On Track(Actual CPNP1 ≤ Target CPNP1, 2026-08-09 사용자 확정 — CPNP1은
    // 낮을수록 좋은 지표) 하이라이트만 적용(32_ACQReportStyles.js).
    CPNP1_COLUMN: 24,  // X열
