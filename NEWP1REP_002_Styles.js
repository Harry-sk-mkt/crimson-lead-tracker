 ...
 * v1.5.0 (2026-08-09)
 * - **New P1 Target%(Q) On Track 강조를 C(Segment)/D(New P1)까지 확장 +
 *   볼드 추가**(사용자 요청 — "Q가 on track이면 C, D, Q를 색칠하고 bold").
 *   기존 `highlightAtOrAboveThreshold_()`(32_ACQReportStyles.js, 옅은 초록
 *   #C6E0B4, 볼드 없음) 호출을 제거하고 신규 `applyNewP1TargetOnTrackHighlight_()`
 *   로 대체 — 밝은 초록(#01ef18)+볼드로 ACQ_REP On Track 계열과 색상 통일.
 *   줄무늬 배경 로직 실행 *이후*에 호출해야 줄무늬가 강조색을 덮어쓰지
 *   않음(순서 주의). 재실행 시 이전 강조가 남지 않도록 배경 초기화 시
 *   `setFontWeight("normal")`도 같이 리셋하도록 변경.
 * v1.4.0 (2026-08-04)
...
function applyNewP1ReportStyles_(sheet, rowCount){
  ...
  if(rowCount > 0){
    ...
    sheet.getRange(startRow, 1, rowCount, totalCols).setBackground(null).setFontWeight("normal");
    sheet.getRange(startRow, targetStartCol, rowCount, targetCols).setBackground(null).setFontWeight("normal");
    ...
    //----------------------------------------------------------
    // New P1 Target On Track(Q) 강조 — 줄무늬보다 반드시 나중에 실행해야
    // 줄무늬가 이 강조색을 덮어쓰지 않는다(2026-08-09 사용자 요청 —
    // "Q가 on track이면 C, D, Q를 색칠하고 bold"). 기존
    // highlightAtOrAboveThreshold_()(옅은 초록, 볼드 없음)를 대체 —
    // ACQ_REP의 On Track 계열(32_ACQReportStyles.js)과 동일하게 밝은
    // 초록(#01ef18)+볼드로 통일.
    //----------------------------------------------------------

    applyNewP1TargetOnTrackHighlight_(sheet, startRow, rowCount, targetStartCol + 3);
...
/**
 * ==========================================================
 * Apply NewP1 Target On Track Highlight (Q On Track → C/D/Q 강조+볼드)
 *
 * WHY (2026-08-09 사용자 요청)
 * New P1 Target%(Q, percentCol)가 100% 이상(On Track)이면, 그 행을 스캔하기
 * 쉽도록 Segment(C)/New P1(D)/Q 자체까지 같이 밝은 초록(#01ef18)+볼드로
 * 강조한다 — ACQ_REP의 "Revenue On Track이면 Segment도 강조"와 동일 취지
 * (32_ACQReportStyles.js buildACQReportBackgrounds_() 참고). rowCount가
 * 크지 않은 리포트라(월×세그먼트 단위 집계) On Track인 행만 개별
 * setBackground()/setFontWeight() 호출 — highlightAtOrAboveThreshold_()
 * (32_ACQReportStyles.js)와 동일한 호출 비용 패턴.
 *
 * INPUT
 * sheet : Sheet
 * startRow : Number  (데이터 시작 행)
 * rowCount : Number  (데이터 행 수)
 * percentCol : Number  (New P1 Target% 컬럼, 1-based — Q)
 *
 * SIDE EFFECT
 * On Track인 행의 C(3)/D(4)/percentCol 셀 배경·볼드 변경. 나머지 행은
 * applyNewP1ReportStyles_()가 이미 배경 null·폰트 normal로 초기화해둔
 * 상태를 그대로 둠(이 함수는 건드리지 않음).
 * ==========================================================
 */
function applyNewP1TargetOnTrackHighlight_(sheet, startRow, rowCount, percentCol){

  if(rowCount === 0) return;

  const ON_TRACK_COLOR = "#01ef18";
  const SEGMENT_COL = 3;   // C
  const NEWP1_COL = 4;     // D

  const values = sheet.getRange(startRow, percentCol, rowCount, 1).getValues();
  ...
  for(let i = 0; i < rowCount; i++){
    ...
    if(raw !== "" && Number(raw) >= 1){
      ...
      sheet.getRange(row, SEGMENT_COL).setBackground(ON_TRACK_COLOR).setFontWeight("bold");
      sheet.getRange(row, NEWP1_COL).setBackground(ON_TRACK_COLOR).setFontWeight("bold");
      sheet.getRange(row, percentCol).setBackground(ON_TRACK_COLOR).setFontWeight("bold");
...
function annotateNewP1ReportMetricNotes_(sheet, headerRow){
  ...
  targetNotes[t + 3] = "New P1 Target% — New P1(4) ÷ New P1 Target. 100% 이상(On Track)이면 이 셀뿐 아니라 Segment(C)/New P1(D)도 같이 초록+볼드 강조(2026-08-09).";
